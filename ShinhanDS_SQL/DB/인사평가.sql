...
----------------------------------------------------------------------
--performances점수 데이터 삽입
INSERT INTO performances (review_id, employee_id, check_id, score, review_year)
VALUES (
...
--점수 조회
SELECT p.review_id, c.rank, c.department, p.employee_id, c.description, p.review_year, p.score
FROM performances p
INNER JOIN checkLists c ON p.check_id = c.check_id
WHERE p.employee_id = 101;
...
SELECT p.employee_id, p.review_year, ROUND(AVG(p.score), 2) AS average_score
FROM performances p
INNER JOIN checkLists c ON p.check_id = c.check_id
WHERE p.employee_id = 101
GROUP BY p.employee_id, p.review_year;

--평균점수 뷰
CREATE OR REPLACE VIEW avg_scores AS
SELECT p.employee_id, p.review_year, ROUND(AVG(p.score), 2) AS average_score
FROM performances p
INNER JOIN checkLists c ON p.check_id = c.check_id
GROUP BY p.employee_id, p.review_year;
...
--뷰 불러오기
SELECT * FROM avg_scores WHERE employee_id = 201;
